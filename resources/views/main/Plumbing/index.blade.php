...
@section('title', 'Buildin Plumbing')

@section('content')
<div class="flex justify-between items-center mb-6">
    <h1 class="text-2xl font-bold text-gray-800">Building Plumbing</h1>
    <div class="flex space-x-2">
        <form method="GET" action="{{ route('plumbings.index') }}" class="inline">
...
@foreach ($Plumbing as $plumbings)

<div class="bg-white rounded-lg shadow-md p-6 mb-6">
    <div class="flex justify-between items-center mb-4">
        <h2 class="text-xl font-semibold">Inventory Management</h2>
        <form action="{{ route('plumbings.index') }}" method="GET" class="flex items-center space-x-2">
            <input type="text" name="search" value="{{ request('search') }}" placeholder="Search plumbing..."
        ...
        <table class="min-w-full divide-y divide-gray-200">
            ...
            <tbody class="bg-white divide-y divide-gray-200">
                <tr>
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $plumbings->name }}</td>
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                        <span class="px-2 py-1 bg-green-100 text-green-800 rounded-full text-xs">{{ $plumbings->stock }}
                            in stock</span>
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                        Rp{{ number_format($plumbings->price, 0, ',', '.') }}
                    </td>
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                        ...
                        <form action="{{ route('plumbings.destroy', $plumbings->id) }}" method="POST" class="inline">
                            @csrf
                            @method('DELETE')
    ...
    <div class="flex justify-between items-center mt-4">
        <div class="text-sm text-gray-500">
            Showing {{ $Plumbing->firstItem() }} to {{ $Plumbing->lastItem() }} of {{ $Plumbing->total() }}
            entries
        </div>

        <div class="flex space-x-1">
            @if ($Plumbing->onFirstPage())
            <span class="px-3 py-1 border rounded-md text-sm text-gray-400">Previous</span>
            @else
            <a href="{{ $Plumbing->previousPageUrl() }}" class="px-3 py-1 border rounded-md text-sm">Previous</a>
            @endif

            @for ($i = 1; $i <= $Plumbing->lastPage(); $i++)
                <a href="{{ $Plumbing->url($i) }}"
                    class="px-3 py-1 border rounded-md text-sm {{ $Plumbing->currentPage() == $i ? 'bg-blue-600 text-white' : '' }}">
                    {{ $i }}
                </a>
                @endfor

                @if ($Plumbing->hasMorePages())
                <a href="{{ $Plumbing->nextPageUrl() }}" class="px-3 py-1 border rounded-md text-sm">Next</a>
                @else
...
<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 ">
    ...
    <div
        ...
        <div class="relative">
            <img src="{{ asset('storage/' . $plumbings->path) }}" alt="Product" class="w-full h-48 object-cover">
            <div
        ...
        <div class="p-4">
            <div class="flex justify-between items-start">
                <div>
                    <h3 class="font-bold text-lg mb-1">{{ $plumbings->name }}</h3>
                    <p class="text-gray-600 text-sm">{{ $plumbings->description }}</p>
                </div>
                <div class="text-right">
                    <span
                        class="text-blue-600 font-bold block">Rp{{ number_format($plumbings->price, 0, ',', '.') }}</span>
                </div>
            </div>
            <div class="mt-3 flex justify-between items-center">
                <div>
                    @if ($plumbings->stock < 5) <span class="text-yellow-600 text-sm font-medium">
                        <i class="fas fa-exclamation-circle mr-1"></i>
                        Low Stock ({{ $plumbings->stock }})
                        </span>
                        @else
                        <span class="text-green-600 text-sm font-medium">
                            <i class="fas fa-check-circle mr-1"></i>
                            In Stock ({{ $plumbings->stock }})
                        </span>
                        @endif
                </div>
...
<div id="editProductModal-{{ $plumbings->id }}"
    ...
    <div class="bg-white w-full max-w-lg p-6 rounded-xl shadow-lg relative">
        ...
        <form action="{{ route('plumbings.update', $plumbings->id) }}" method="POST" enctype="multipart/form-data"
            class="space-y-4">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <input type="text" name="name" value="{{ $plumbings->name }}" id="name" required
            ...
            <div>
                ...
                <input type="number" name="stock" value="{{ $plumbings->stock }}" id="stock" required
            ...
            <div>
                ...
                <input type="number" name="price" id="editPrice" value="{{ $plumbings->price }}" step="0.01" required
            ...
            <div>
                ...
                <textarea name="description" id="description" rows="3" placeholder="Enter product description"
                    class="mt-1 block w-full rounded-md border-gray-300 shadow-sm py-2 px-3 text-lg focus:border-blue-500 focus:ring-blue-500">{{ $plumbings->description }}</textarea>
            </div>
            <div>
                ...
                <select name="status" id="status" required
                    ...
                    <option value="1" {{ $plumbings->status == 1 ? 'selected' : '' }}>Active</option>
                    <option value="0" {{ $plumbings->status == 0 ? 'selected' : '' }}>Inactive</option>
            ...
            <div class="flex justify-end space-x-2 mt-4">
                <button type="button" onclick="closeModalEdit('{{ $plumbings->id }}')"
        ...
        <!-- Close Button -->
        <button onclick="closeModalEdit('{{ $plumbings->id }}')"
...
<div id="addProductModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 hidden">
    ...
    <div class="bg-white w-full max-w-lg p-6 rounded-xl shadow-lg relative">
        ...
        <form action="{{ route('plumbings.store') }}" method="POST" enctype="multipart/form-data" class="space-y-4">
            @csrf
